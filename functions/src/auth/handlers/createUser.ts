/**
 * Create User Cloud Function
 *
 * Admin-only function to create a new user in the system.
 * User is created in "pending" state and requires invitation acceptance.
 */

import * as functions from "firebase-functions/v2/https";
import {
  handleCorsAndMethod,
  parseBody,
  sendError,
  sendSuccess,
} from "../../common/helpers/httpHelpers";
import {validateRequiredFields} from "../validators/requestValidator";
import {validateAdmin} from "../validators/adminValidator";
import {fail} from "../../common/types/result";
import * as userService from "../services/userService";

interface CreateUserRequest {
  displayName: string;
  role: "admin" | "user";
  adminUserId: string;
  adminDeviceId: string;
}

export const createUser = functions.onRequest(async (req, res) => {
  // Handle CORS and validate method
  if (!handleCorsAndMethod(req, res, "POST")) return;

  // Parse request body
  const bodyResult = parseBody<CreateUserRequest>(req.body);
  if (!bodyResult.success) {
    return sendError(res, bodyResult);
  }

  const {displayName, role, adminUserId, adminDeviceId} = bodyResult.data;

  // Validate required fields
  const validationResult = validateRequiredFields(bodyResult.data, [
    "displayName",
    "role",
    "adminUserId",
    "adminDeviceId",
  ]);
  if (!validationResult.success) {
    return sendError(res, validationResult);
  }

  // Validate role
  if (role !== "admin" && role !== "user") {
    return sendError(res, fail("role debe ser 'admin' o 'user'", 400));
  }

  // Validate admin credentials
  const adminResult = await validateAdmin(adminUserId, adminDeviceId);
  if (!adminResult.success) {
    return sendError(res, adminResult);
  }

  // Execute business logic
  const result = await userService.createUser(displayName, role, adminUserId);

  if (!result.success) {
    return sendError(res, result);
  }

  // Send success response
  return sendSuccess(res, result.data);
});
